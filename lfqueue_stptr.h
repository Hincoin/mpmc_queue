...
private:
    ...
    struct alignas(alignment) aligned_atomic_ptr
	{
		...
		alignas(alignment) std::atomic<bool> dirty_; // Not necessary to be atomic<bool> because this value is only written to when the owning pointer is visible as 
					// nullptr to other threads. A non-owning thread will only read dirty_ when ptr is non-nullptr ( and thus, not being written to).
...
public:
	...
	struct queue_holder
	{
		...
		~queue_holder()
		{
			if(ptr_)
			{
				data_[index_].dirty_.store(queue().size() > 0, std::memory_order_relaxed);
	...
	{	
		...
		// static thread_local std::uniform_int_distribution<> distr(0,num_queues);
		// static thread_local int index = distr(generator);
		// return index;
	...
	HINLINE queue_holder acquire_queue_dequeue()
	{
		...
		for(int i = 0; i < num_queues; ++i)
		{
			...
			if(! data_[index].load(std::memory_order_relaxed))
				continue;
	...
	//(?:\s*?template[\s\S]*?\<.*\>)\s+?[\w]+?\s+?[\w]+?\s*\([\s\S]*?\)[\s]*?\{[\s\S]*?\}
	...
	~lf_queue()
	{
		...
		{
			delete reinterpret_cast<pointer>(reinterpret_cast<uintptr_t>(st_queue.load(std::memory_order_relaxed)) & ~1);
		}
	}
